export const supplierInvoiceTemplate = (invoice) => {
  ...
  <title>Commande fournisseur ${invoice.invoiceNumber || "-"}</title>
  <style>
    :root {
    ...
    <div class="main-content">
      <header>
        <div class="logo-container">
          ${invoice.logo ? `<img src="${invoice.logo}" alt="Logo" />` : `<div style="height: 40px;"></div>`}
        </div>
        <div class="invoice-info">
          <h1>COMMANDE FOURNISSEUR</h1>
          <div style="font-weight: 600; color: var(--secondary-color); font-size: 14px;">Réf: ${invoice.invoiceNumber || "-"}</div>
          <div style="color: var(--text-light); font-size: 12px;">Date: ${invoice.date ? new Date(invoice.date).toLocaleDateString() : "-"}</div>
        </div>
      </header>
      ...
      <div class="bill-to-section">
        <div class="address-block">
          ...
          ${formatAddress(invoice.supplier?.address)}
        </div>
      </div>
      ...
      <div class="totals-wrapper">
        <div class="totals">
          <div class="total-row">
            ...
            <span>${(invoice.subtotal || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })} TND</span>
          </div>
          <div class="total-row">
            <span style="color: var(--text-light);">TVA</span>
            <span>${(invoice.totalVAT || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })} TND</span>
          </div>
          <div class="total-row">
            <span style="color: var(--text-light);">Taxes</span>
            <span>${(invoice.totalTaxes || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })} TND</span>
          </div>
          <div class="total-row">
            <span style="color: var(--text-light);">Timbre Fiscal</span>
            <span>${(invoice.timbreFiscal || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })} TND</span>
          </div>
          <div class="total-row grand-total">
            <span>Net à payer</span>
            <span>${(invoice.netPay || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })} TND</span>
          </div>
        </div>
      </div>
      ...
      <div class="note-section">
        ...
        <div style="font-size: 13px; color: var(--text-main);">
          ${invoice.note || "Merci pour votre confiance !"}
        </div>
      </div>
    </div>
    ...
    <div class="footer-section">
      <div class="footer-content">
        <div class="issuer-info address-block">
          ...
          <div style="font-size: 12px;">${formatAddress(invoice.company?.address)}</div>
        </div>
