...
const AllSupplierOrders = () => {
  ...
  // 🔹 Generate dynamic years based on order data
  ...
  <main className="flex-1 flex flex-col min-w-0">
    {/* HEADER */}
    <header className="h-20 bg-white dark:bg-[#1F2937] border-b border-slate-200 dark:border-slate-700 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-40">
      <div className="flex items-center gap-4">
        <button
          onClick={() => setIsSidebarOpen(true)}
          className="lg:hidden p-2 text-slate-500 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-[#374151] rounded-lg transition-colors"
        >
          <Menu size={22} />
        </button>
        ...
        <div>
          ...
          <p className="hidden sm:block text-xs font-medium text-slate-400 dark:text-slate-500">
            Gérer vos cycles de facturation et de recouvrement
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative hidden md:block">
          ...
          <input
            ...
            className="pl-10 pr-4 py-2 bg-slate-50 dark:bg-[#1F2937] border border-slate-200 dark:border-slate-700 rounded-xl text-sm dark:text-slate-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none w-64 transition-all"
          />
        </div>
      </div>
    </header>
    ...
    <div className="p-4 lg:p-8 max-w-7xl mx-auto w-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-[#1F2937] p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-slate-50 dark:bg-[#1F2937] border border-slate-200 dark:border-slate-700 text-sm rounded-lg px-3 py-1.5 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500/20 outline-none cursor-pointer"
            >
        ...
        <AnimatePresence >
          {filteredOrders.length === 0 ? (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="bg-white dark:bg-[#1F2937] rounded-3xl border border-slate-200 dark:border-slate-700 p-20 flex flex-col items-center text-slate-400 dark:text-slate-500"
            >
              <div className="p-4 bg-slate-50 dark:bg-[#111827] rounded-full mb-4">
            ...
            currentOrders.map((order) => (
              <motion.div
                ...
                className="group bg-white dark:bg-[#1F2937] p-4 md:px-6 md:py-4 rounded-2xl border border-slate-200 dark:border-slate-700 hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-500/5 dark:hover:shadow-indigo-900/20 transition-all md:grid md:grid-cols-12 items-center gap-4 flex flex-col"
              >
                {/* Order ID & Date */}
                <div className="col-span-2 w-full md:w-auto">
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded-xl bg-slate-50 dark:bg-[#111827] flex items-center justify-center text-slate-400 dark:text-slate-500 group-hover:bg-indigo-50 dark:group-hover:bg-indigo-900/20 group-hover:text-indigo-500 transition-colors">
                      <FileText size={18} />
                    </div>
                    <div>
                ...
                <div className="col-span-3 w-full md:w-auto border-t md:border-t-0 pt-2 md:pt-0">
                  ...
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 truncate">
                    {order.supplier?.name || "Aucun nom de client"}
                  </p>
                </div>
                  ...
                  <span
                    className={`text-[10px] font-extrabold px-3 py-1 rounded-full uppercase tracking-widest ${
                      order.isPaid
                        ...
                        : "bg-rose-100 dark:bg-rose-900/20 text-rose-700"
                    }`}
                  >
                ...
                <div className="col-span-2 w-full md:w-auto text-center md:text-right">
                  ...
                  <span className="text-lg font-black text-slate-900 dark:text-slate-100">
                    ...
                    <span className="text-[10px] text-slate-400 dark:text-slate-500 ml-0.5">TND</span>
                  </span>
                </div>
                ...
                <div className="col-span-3 flex items-center justify-end gap-2 w-full md:w-auto border-t md:border-t-0 pt-3 md:pt-0">
                  <button
                    ...
                    title={order.isPaid ? "Marquer comme non payée" : "Marquer comme payée"}
                  ...
                  >
                    {order.isPaid ? <XCircle size={20} /> : <CheckCircle size={20} />}
                  </button>

                  <button
                    onClick={() => downloadPDF(order._id)}
                    className="p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-[#374151] rounded-xl transition-colors"
                  ...
                  <button
                    ...
                    className="p-2 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-xl transition-colors"
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4 border-t border-slate-200 dark:border-slate-700">
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
            Affichage de <span className="text-slate-900 dark:text-slate-100">{currentOrders.length}</span> sur{" "}
            <span className="text-slate-900 dark:text-slate-100">{filteredOrders.length}</span> enregistrements
          </p>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 px-4 py-2 text-sm font-bold text-slate-600 dark:text-slate-300 bg-white dark:bg-[#1F2937] border border-slate-200 dark:border-slate-700 rounded-xl disabled:opacity-50 hover:bg-slate-50 dark:hover:bg-[#374151] transition-colors"
            >
              <ChevronLeft size={16} />
              Précédent
            </button>
                ...
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${
                    currentPage === i + 1
                      ...
                      : "text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-[#374151]"
                  }`}
                >
                  {i + 1}
                </button>
...
const StatCard = ({ title, value, icon, color, trend }) => (
  <motion.div
    ...
    className="bg-white dark:bg-[#1F2937] p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-5 transition-shadow hover:shadow-xl hover:shadow-slate-200/50 dark:hover:shadow-slate-900/50"
  >
