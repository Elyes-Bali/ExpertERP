...
const AllPlanComptable = () => {
  const { plans, fetchPlans, deletePlan, createPlan, updatePlan } =
    usePlanStore();
  ...
  const handleEdit = (plan) => {
    setForm({ code: plan.code, label: plan.label });
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 dark:text-slate-400">
            Gérez votre structure comptable
          </p>
        </div>
      </div>
      ...
      {totalPages > 1 && (
        <div className="flex justify-between items-center pt-4 border-t border-slate-200 dark:border-gray-700">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {filteredPlans.length} résultats
          </p>
...
export default AllPlanComptable;