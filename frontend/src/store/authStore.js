...
updateIsVerified: async (userId, isVerified) => {
  set({ isLoading: true, error: null });
  try {
    const res = await axios.put(`${API_URL}/update-verified`, {
      userId,
      isVerified,
    });
    set({ isLoading: false });
    ...
    set({
      error: error.response?.data?.message || "Failed to update verification",
      isLoading: false,
    });
    throw error;
  }
...
createUser: async (email, password, name, role) => {
  ...
  try {
    ...
    set({
      isLoading: false,
      message: "User created successfully 🎉", // ✅ global message
    });
...
deleteUser: async (userId) => {
  try {
    ...
    // optional: you can also remove it from local UI state if you store users
    return true;
  } catch (error) {
    console.error(
      "Error deleting user:",
