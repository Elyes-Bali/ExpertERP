   ...
   <main className={mainContentClass}>
    <div className="mt-8 bg-white p-6 rounded-xl shadow-2xl">
     <h3 className="text-3xl font-bold mb-6 flex items-center text-gray-800">
      ...
      Historique des paiements de tous les utilisateurs
     </h3>
     
     {isLoading ? (
      <div className="flex justify-center items-center py-12">
       <Loader2 className="w-8 h-8 animate-spin text-indigo-500 mr-3" />
       <p className="text-lg text-gray-600">Récupération des données de paiement...</p>
      </div>
     ) : (
        ...
        <>
         <table className="min-w-full divide-y divide-gray-200 shadow-md rounded-lg overflow-hidden">
          <thead className="bg-gray-100">
           <tr>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider"><User className="inline w-4 h-4 mr-1"/> Utilisateur</th>
...
{/*            <span className="text-sm text-gray-700">
            ...
            {Math.min(indexOfLastPayment, allPayments.length)}** sur **
        ...
        <p className="text-gray-500 mt-4 text-center p-6 bg-gray-50 rounded-lg">
         Aucun historique de paiement trouvé pour tous les utilisateurs.
        </p>
       )}
      </div>
     )}
